...
export function registerAllHandlers(registerHandler: Function): void {
  ...
  registerHandler(MessageType.GET_SUBTITLE, async (message: any) => {
    ...
    try {
      ...
      console.log(`Processing ${segments.length} subtitle segments for video ${videoId} (fromCache: ${isFromCache})`);
      
      // Get user profile for translation
      const profile = await getUserProfile();
      
      // Process subtitles with LLM translations (will hit translation_cache for cached segments)
      ...
      // Update cache with processed segments to make future loads instantaneous
      const { cacheVideoSession } = await import('../../src/lib/storage-manager');
      try {
        await cacheVideoSession({
          videoId,
        ...
        console.log('Translated subtitle data cached successfully');
      } catch (cacheError) {
        console.warn('Failed to update cache with translated segments:', cacheError);
      }
  ...
  registerHandler(MessageType.TRANSLATE_WORDS, async (message: any) => {
    ...
    try {
      ...
      const profile = await getUserProfile();
      const translationsMap = await translateWords(words, profile);
      ...
      return {
        ...
        translations: words.map((item: any) => ({
          word: item.word || item,
          translation: translationsMap.get(item.word || item) || '(翻译失败)',
        })),
      };
    } catch (error) {
