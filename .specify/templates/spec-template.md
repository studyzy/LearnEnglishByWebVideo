...
- **FR-001**: 扩展必须 [具体能力, 例如: "在视频页面检测英文字幕"]
- **FR-002**: 扩展必须 [具体能力, 例如: "提供单词翻译和发音"]
- **FR-003**: 用户必须能够 [关键交互, 例如: "保存生词到个人词库"]
- **FR-004**: 扩展必须 [数据需求, 例如: "使用chrome.storage.local持久化用户配置"]
- **FR-005**: 扩展必须 [行为, 例如: "遵循Manifest V3规范"]

**Chrome扩展特定需求**:
- **FR-006**: 必须使用Service Worker作为后台脚本
- **FR-007**: content scripts必须按需注入，避免性能影响
- **FR-008**: 必须最小化权限请求，遵循最小权限原则
- **FR-009**: 所有Chrome API调用必须包含错误处理
